...
@SystemForm(name = "Watch History", description = "Your movie watch history")
public class FormWatchHistory extends Form {

    private JPanel gridPanel;
    private JScrollPane scrollPane;
    private List<WatchHistoryResponse> historyList = new ArrayList<>();
    ...
    private JPanel createHeader() {
        ...
        JLabel title = new JLabel("Lịch sử xem");
        ...
        panel.add(new JLabel("(" + historyList.size() + " phim)"), "aligny center");
    ...
    private void loadWatchHistory() {
        ...
        // Load in background
        new SwingWorker<ApiResponse<List<WatchHistoryResponse>>, Void>() {
            @Override
            protected ApiResponse<List<WatchHistoryResponse>> doInBackground() {
                return WatchHistoryApi.getUserWatchHistory(getCurrentUserId(), currentPage, pageSize);
            }

            @Override
            protected void done() {
                try {
                    ApiResponse<List<WatchHistoryResponse>> response = get();
                    if (response != null && response.getCode() == 200) {
                        historyList = response.getResult();
                        hasMore = historyList.size() >= pageSize;
                        displayHistory();
                        updateHeader();
                        updatePagination();
                    } else {
                        showError("Lỗi tải lịch sử xem: " + (response != null ? response.getMessage() : "Unknown"));
                    }
                } catch (Exception e) {
    ...
    private void displayHistory() {
        ...
        if (historyList.isEmpty()) {
            ...
            JLabel emptyText = new JLabel("Chưa có lịch sử xem");
            emptyText.putClientProperty(FlatClientProperties.STYLE, "font:bold +3;foreground:$Label.disabledForeground;");
            
            JLabel emptyHint = new JLabel("Xem chi tiết phim để lưu lịch sử");
            ...
            for (WatchHistoryResponse history : historyList) {
                gridPanel.add(createHistoryCard(history));
            }
        }
    ...
    private JPanel createHistoryCard(WatchHistoryResponse history) {
        MovieResponse movie = history.getMovie();
        ...
        JLabel lastWatched = new JLabel("Xem lần cuối: " + history.getUpdatedAt().format(DATE_FORMATTER));
        lastWatched.putClientProperty(FlatClientProperties.STYLE, "font:-1;foreground:$Label.disabledForeground;");
        ...
        JButton btnRemove = new JButton("🗑️");
        btnRemove.putClientProperty(FlatClientProperties.STYLE, "arc:12;");
        btnRemove.setToolTipText("Xóa khỏi lịch sử");
        btnRemove.addActionListener(e -> removeHistory(history.getId(), movie));
    ...
    private void removeHistory(Long historyId, MovieResponse movie) {
        int confirm = JOptionPane.showConfirmDialog(
                this,
                "Bạn có chắc muốn xóa \"" + movie.getTitle() + "\" khỏi lịch sử?",
                "Xác nhận",
        ...
        if (confirm == JOptionPane.YES_OPTION) {
            new SwingWorker<ApiResponse<Void>, Void>() {
                ...
                protected ApiResponse<Void> doInBackground() {
                    return WatchHistoryApi.removeWatchHistory(getCurrentUserId(), movie.getId());
                }
                ...
                protected void done() {
                    try {
                        ...
                        if (response != null && response.getCode() == 200) {
                            Toast.show(FormWatchHistory.this, Toast.Type.SUCCESS, "Đã xóa khỏi lịch sử");
                            loadWatchHistory(); // Reload
                        } else {
                            Toast.show(FormWatchHistory.this, Toast.Type.ERROR, 
                                    "Lỗi: " + (response != null ? response.getMessage() : "Unknown"));
                        }
                    } catch (Exception e) {
                        Toast.show(FormWatchHistory.this, Toast.Type.ERROR, "Lỗi: " + e.getMessage());
                        e.printStackTrace();
                    }
                }
            }.execute();
